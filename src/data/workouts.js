// Modified Greyskull LP - Personalized Workout Program
// Based on research: neural strength, 1-5 rep range, 3-5 min rest, deload every 6 weeks

export const userProfile = {
  height: "5'10\"",
  heightCm: 178,
  weightKg: 72.7,
  weightLbs: 160,
  tdee: 2700,
  bmr: 1720,
};

export const exercises = {
  squat: {
    id: 'squat',
    name: 'Barbell Back Squat',
    nameShort: 'Squat',
    muscle: 'Legs, Core, Back',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'compound',
    formTips: [
      'Feet shoulder-width, toes slightly out',
      'Break at hips first, then knees',
      'Keep chest up, back neutral',
      'Depth: hip crease below knee (parallel+)',
      'Drive through heels, push knees out',
      'WHITE-KNUCKLE the bar — full body tension',
    ],
    warnings: [
      'NEVER round your lower back under load',
      'Knees caving inward = STOP and reduce weight',
      'If you lose tension at the bottom, the weight is too heavy',
    ],
    startWeight: 20,
    increment: 2.5,
  },
  deadlift: {
    id: 'deadlift',
    name: 'Barbell Deadlift',
    nameShort: 'Deadlift',
    muscle: 'Back, Legs, Core, Grip',
    category: 'strength',
    muscleGroup: 'back',
    type: 'compound',
    formTips: [
      'Bar over mid-foot, shins close to bar',
      'Hip hinge — push hips back, not down',
      'Shoulders slightly in front of the bar',
      'Engage lats — "protect your armpits"',
      'Drive through the floor, lock hips at top',
      'CRUSH the bar — irradiation from hands to core',
    ],
    warnings: [
      'NEVER round your back — this is the #1 cause of back injuries',
      'Do NOT jerk the bar off the floor — build tension first',
      'If grip fails before muscles, use mixed grip',
    ],
    startWeight: 40,
    increment: 2.5,
  },
  bench: {
    id: 'bench',
    name: 'Barbell Bench Press',
    nameShort: 'Bench Press',
    muscle: 'Chest, Shoulders, Triceps',
    category: 'strength',
    muscleGroup: 'chest',
    type: 'compound',
    formTips: [
      'Retract shoulder blades — squeeze them together',
      'Slight arch in lower back, feet flat on floor',
      'Grip slightly wider than shoulder-width',
      'Lower to mid-chest, elbows at 45° (NOT flared)',
      'Press up and slightly back toward face',
      'SQUEEZE the bar like you\'re trying to snap it in half',
    ],
    warnings: [
      'Flared elbows = shoulder impingement — tuck to 45°',
      'ALWAYS use a spotter or safety pins for heavy sets',
      'Never bounce bar off chest — use pause reps for real strength',
    ],
    startWeight: 20,
    increment: 1.25,
  },
  ohp: {
    id: 'ohp',
    name: 'Barbell Overhead Press',
    nameShort: 'Overhead Press',
    muscle: 'Shoulders, Triceps, Core',
    category: 'strength',
    muscleGroup: 'shoulders',
    type: 'compound',
    formTips: [
      'Start with bar at collarbone, elbows in front',
      'Squeeze glutes and brace core HARD',
      'Press straight up, move head back then forward',
      'Lockout overhead — bar over mid-foot',
      'Full body tension — this is a full body lift',
      'Controlled descent back to starting position',
    ],
    warnings: [
      'Do NOT lean back excessively — keep core braced',
      'This lift progresses SLOWEST — use 1.25kg increments',
      'Pain in shoulders? Check mobility before adding weight',
    ],
    startWeight: 20,
    increment: 1.25,
  },
  pullup: {
    id: 'pullup',
    name: 'Weighted Pull-ups',
    nameShort: 'Pull-ups',
    muscle: 'Back, Biceps, Core, Grip',
    category: 'calisthenics',
    muscleGroup: 'back',
    type: 'compound',
    formTips: [
      'Grip slightly wider than shoulder-width',
      'Start from full dead hang — arms straight',
      'Pull elbows down and back toward hips',
      'Chin over bar at the top',
      'Slow, controlled descent (3 seconds)',
      'Add weight belt when bodyweight reps > 8',
    ],
    warnings: [
      'No kipping — strict form only',
      'Can\'t do 5? Use bands for assistance, NOT the machine',
      'Shoulder pain at bottom? Don\'t go to full dead hang yet',
    ],
    startWeight: 0,
    increment: 1.25,
  },
  row: {
    id: 'row',
    name: 'Barbell Bent-Over Row',
    nameShort: 'Bent-Over Row',
    muscle: 'Back, Biceps, Rear Delts',
    category: 'strength',
    muscleGroup: 'back',
    type: 'compound',
    formTips: [
      'Hinge at hips — 45° angle torso',
      'Bar hangs at arm\'s length below shoulders',
      'Pull to lower chest / upper abs',
      'Squeeze shoulder blades at the top',
      'Keep core braced — no rocking',
      'Lower with control',
    ],
    warnings: [
      'Do NOT jerk the weight up — if you must jerk, it\'s too heavy',
      'Keep neutral spine — same rules as deadlift',
      'This is NOT an ego lift — strict form matters more than weight',
    ],
    startWeight: 30,
    increment: 1.25,
  },
  curl: {
    id: 'curl',
    name: 'Barbell Curl',
    nameShort: 'Barbell Curl',
    muscle: 'Biceps, Forearms',
    category: 'strength',
    muscleGroup: 'biceps',
    type: 'accessory',
    formTips: [
      'Stand straight, elbows pinned to sides',
      'Curl with control — no body swing',
      'Squeeze at the top for 1 second',
      'Slow eccentric (3 sec down)',
    ],
    warnings: [
      'Swinging = too heavy — drop the weight',
    ],
    startWeight: 15,
    increment: 1.25,
  },
  farmerWalk: {
    id: 'farmerWalk',
    name: "Farmer's Walk",
    nameShort: "Farmer's Walk",
    muscle: 'Grip, Core, Shoulders, Full Body',
    category: 'cardio',
    muscleGroup: 'fullbody',
    type: 'conditioning',
    formTips: [
      'Pick up heavy dumbbells, one in each hand',
      'Stand tall — shoulders back, chest up',
      'Walk with short, quick steps',
      'Maintain tight core and neutral spine',
      'CRUSH the handles — irradiation effect',
    ],
    warnings: [],
    startWeight: 16,
    increment: 2,
  },
  kbSwing: {
    id: 'kbSwing',
    name: 'Kettlebell Swing',
    nameShort: 'KB Swing',
    muscle: 'Hips, Glutes, Core, Shoulders',
    category: 'cardio',
    muscleGroup: 'core',
    type: 'conditioning',
    formTips: [
      'Hip hinge movement — NOT a squat',
      'Drive hips forward explosively',
      'Arms are just straps — power from hips',
      'Keep core braced throughout',
      'Control the swing on the way down',
    ],
    warnings: [
      'Do NOT squat the swing — it\'s a HIP HINGE',
    ],
    startWeight: 12,
    increment: 4,
  },
  weightedDips: {
    id: 'weightedDips',
    name: 'Weighted Chest Dips',
    nameShort: 'Weighted Dips',
    muscle: 'Chest, Shoulders, Triceps',
    category: 'calisthenics',
    muscleGroup: 'chest',
    type: 'compound',
    formTips: [
      'Grip bars firmly, keep elbows slightly tucked',
      'Leaning forward hits chest; upright hits triceps',
      'Lower until shoulders are below elbows',
      'Drive up explosively, lock out at top',
      'White-knuckle the bars to recruit shoulder stability',
    ],
    warnings: [
      'Avoid shoulder shrugging — keep shoulders packed down',
      'If you feel shoulder joint pain, reduce depth or weight',
    ],
    startWeight: 0,
    increment: 1.25,
  },
  frontSquat: {
    id: 'frontSquat',
    name: 'Barbell Front Squat',
    nameShort: 'Front Squat',
    muscle: 'Quads, Upper Back, Core',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'compound',
    formTips: [
      'Bar rests on front deltoids, elbows high',
      'Keep torso upright throughout the lift',
      'Squat deep — hip crease below knees',
      'Drive elbows up constantly to prevent bar roll',
      'Brace core like you are taking a punch',
    ],
    warnings: [
      'Elbows dropping = back rounding. Keep elbows high!',
      'Knees caving inward = reduce weight',
    ],
    startWeight: 20,
    increment: 2.5,
  },
  romanianDeadlift: {
    id: 'romanianDeadlift',
    name: 'Barbell Romanian Deadlift',
    nameShort: 'Romanian DL',
    muscle: 'Hamstrings, Glutes, Lower Back',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'compound',
    formTips: [
      'Soft bend in knees, hinge backward at hips',
      'Keep bar sliding down your thighs close to body',
      'Hinge until you feel a deep stretch in hamstrings',
      'Squeeze glutes to pull yourself back up tall',
      'Neutral spine — do not look up or round back',
    ],
    warnings: [
      'This is not a regular deadlift — do not touch floor',
      'NEVER round the lower back under load',
    ],
    startWeight: 30,
    increment: 2.5,
  },
  pendlayRow: {
    id: 'pendlayRow',
    name: 'Barbell Pendlay Row',
    nameShort: 'Pendlay Row',
    muscle: 'Upper Back, Lats, Grip, Core',
    category: 'strength',
    muscleGroup: 'back',
    type: 'compound',
    formTips: [
      'Back completely flat, parallel to the floor',
      'Pull bar explosively from the floor to upper abs',
      'Squeeze shoulder blades, touch chest each rep',
      'Return bar to dead stop on the floor each rep',
      'Keep torso strict — no hip swing or standing up',
    ],
    warnings: [
      'Jerking torso up destroys the neural benefit — stay flat',
      'Lower back rounding is highly dangerous here',
    ],
    startWeight: 30,
    increment: 2.5,
  },
  chinup: {
    id: 'chinup',
    name: 'Weighted Chin-ups',
    nameShort: 'Chin-ups',
    muscle: 'Lats, Biceps, Core, Grip',
    category: 'calisthenics',
    muscleGroup: 'biceps',
    type: 'compound',
    formTips: [
      'Underhand grip (palms facing you), shoulder-width',
      'Start from full dead hang with straight arms',
      'Pull chest up to the bar — squeeze shoulder blades',
      'Lower slowly and with absolute control',
      'Squeeze the bar with maximum white-knuckle force',
    ],
    warnings: [
      'No swinging or kicking legs — strict pull only',
      'Can\'t do 5? Practice bodyweight chin-ups first',
    ],
    startWeight: 0,
    increment: 1.25,
  },
  dumbbellPress: {
    id: 'dumbbellPress',
    name: 'Dumbbell Bench Press',
    nameShort: 'Dumbbell Bench',
    muscle: 'Chest, Shoulders, Triceps',
    category: 'strength',
    muscleGroup: 'chest',
    type: 'compound',
    formTips: [
      'Bring dumbbells to chest, elbows at 45 degrees',
      'Press dumbbells up while squeezing chest',
      'Keep feet planted firmly on the floor'
    ],
    warnings: [],
    startWeight: 12,
    increment: 2,
  },
  inclineDbPress: {
    id: 'inclineDbPress',
    name: 'Incline Dumbbell Press',
    nameShort: 'Incline DB Press',
    muscle: 'Upper Chest, Shoulders',
    category: 'strength',
    muscleGroup: 'chest',
    type: 'compound',
    formTips: [
      'Set incline bench to 30-45 degrees',
      'Press dumbbells straight up above upper chest',
      'Slow controlled eccentric phase'
    ],
    warnings: [],
    startWeight: 10,
    increment: 2,
  },
  latPulldown: {
    id: 'latPulldown',
    name: 'Lat Pulldown Machine',
    nameShort: 'Lat Pulldown',
    muscle: 'Lats, Mid-Back, Biceps',
    category: 'strength',
    muscleGroup: 'back',
    type: 'accessory',
    formTips: [
      'Pull bar down to upper chest, chest up',
      'Squeeze shoulder blades together at bottom',
      'Avoid leaning back excessively'
    ],
    warnings: [],
    startWeight: 30,
    increment: 5,
  },
  seatedCableRow: {
    id: 'seatedCableRow',
    name: 'Seated Cable Row',
    nameShort: 'Cable Row',
    muscle: 'Mid-Back, Lats, Biceps',
    category: 'strength',
    muscleGroup: 'back',
    type: 'accessory',
    formTips: [
      'Keep back flat, sit tall',
      'Pull handle to lower ribs, squeeze lats',
      'Extend arms fully with control'
    ],
    warnings: [],
    startWeight: 25,
    increment: 5,
  },
  lateralRaise: {
    id: 'lateralRaise',
    name: 'Dumbbell Lateral Raise',
    nameShort: 'Lateral Raise',
    muscle: 'Lateral Shoulders',
    category: 'strength',
    muscleGroup: 'shoulders',
    type: 'accessory',
    formTips: [
      'Raise dumbbells out to sides, elbows slightly bent',
      'Lead with elbows, keep pinkies slightly up',
      'Do not swing torso to lift weights'
    ],
    warnings: [],
    startWeight: 4,
    increment: 1,
  },
  facePull: {
    id: 'facePull',
    name: 'Cable Face Pull',
    nameShort: 'Face Pull',
    muscle: 'Rear Delts, Rotator Cuff, Upper Back',
    category: 'strength',
    muscleGroup: 'shoulders',
    type: 'accessory',
    formTips: [
      'Set cable at forehead height',
      'Pull rope toward face, flaring elbows out',
      'Perform external rotation at end of pull'
    ],
    warnings: [],
    startWeight: 10,
    increment: 2.5,
  },
  hammerCurl: {
    id: 'hammerCurl',
    name: 'Dumbbell Hammer Curl',
    nameShort: 'Hammer Curl',
    muscle: 'Brachialis, Biceps, Forearms',
    category: 'strength',
    muscleGroup: 'biceps',
    type: 'accessory',
    formTips: [
      'Hold dumbbells with palms facing each other (neutral)',
      'Curl up with control, keep elbows pinned',
      'Fully extend at bottom'
    ],
    warnings: [],
    startWeight: 8,
    increment: 2,
  },
  tricepPushdown: {
    id: 'tricepPushdown',
    name: 'Cable Tricep Pushdown',
    nameShort: 'Tricep Pushdown',
    muscle: 'Triceps',
    category: 'strength',
    muscleGroup: 'triceps',
    type: 'accessory',
    formTips: [
      'Keep elbows pinned to sides, hands shoulder-width',
      'Extend arms downwards fully, lock out triceps',
      'Squeeze triceps for 1s at bottom'
    ],
    warnings: [],
    startWeight: 15,
    increment: 2.5,
  },
  skullCrusher: {
    id: 'skullCrusher',
    name: 'EZ Bar Skull Crusher',
    nameShort: 'Skull Crusher',
    muscle: 'Triceps',
    category: 'strength',
    muscleGroup: 'triceps',
    type: 'accessory',
    formTips: [
      'Lie on bench, hold bar up with vertical arms',
      'Lower bar to forehead by bending only at elbows',
      'Extend arms back to starting position'
    ],
    warnings: [],
    startWeight: 15,
    increment: 2.5,
  },
  legPress: {
    id: 'legPress',
    name: 'Leg Press Machine',
    nameShort: 'Leg Press',
    muscle: 'Quads, Hamstrings, Glutes',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'accessory',
    formTips: [
      'Feet hip-width on sled, lower with control',
      'Keep lower back pressed flat into seat pad',
      'Do not lock out knees at the top'
    ],
    warnings: ['NEVER lock your knees out under load!'],
    startWeight: 80,
    increment: 10,
  },
  legCurl: {
    id: 'legCurl',
    name: 'Lying Leg Curl',
    nameShort: 'Leg Curl',
    muscle: 'Hamstrings',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'accessory',
    formTips: [
      'Lie face down, pad against lower calves',
      'Curl legs up toward glutes, squeeze hamstrings',
      'Lower back down slowly with control'
    ],
    warnings: [],
    startWeight: 20,
    increment: 5,
  },
  calfRaise: {
    id: 'calfRaise',
    name: 'Standing Calf Raise',
    nameShort: 'Calf Raise',
    muscle: 'Calves',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'accessory',
    formTips: [
      'Stand on edge of platform, drop heels low',
      'Push up onto toes as high as possible',
      'Hold contraction at the top for 1s'
    ],
    warnings: [],
    startWeight: 20,
    increment: 5,
  },
  pushup: {
    id: 'pushup',
    name: 'Bodyweight Push-up',
    nameShort: 'Push-up',
    muscle: 'Chest, Shoulders, Triceps, Core',
    category: 'calisthenics',
    muscleGroup: 'chest',
    type: 'compound',
    formTips: [
      'Hands slightly wider than shoulders, body straight',
      'Lower chest to floor, elbows at 45 degrees',
      'Push up to full lockout, keep core tight'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  declinePushup: {
    id: 'declinePushup',
    name: 'Decline Push-up',
    nameShort: 'Decline Push-up',
    muscle: 'Upper Chest, Shoulders, Triceps',
    category: 'calisthenics',
    muscleGroup: 'chest',
    type: 'compound',
    formTips: [
      'Elevate feet on box or bench, hands on floor',
      'Lower chest to floor, push up to lockout',
      'Keep body straight — no saggy hips'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  dips: {
    id: 'dips',
    name: 'Bodyweight Chest Dips',
    nameShort: 'Dips',
    muscle: 'Chest, Shoulders, Triceps',
    category: 'calisthenics',
    muscleGroup: 'triceps',
    type: 'compound',
    formTips: [
      'Support weight on parallel bars, lower torso with control',
      'Lean slightly forward to target chest',
      'Push back up to lockout'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  invertedRow: {
    id: 'invertedRow',
    name: 'Inverted Row (Bodyweight)',
    nameShort: 'Inverted Row',
    muscle: 'Lats, Upper Back, Biceps',
    category: 'calisthenics',
    muscleGroup: 'back',
    type: 'compound',
    formTips: [
      'Lie under low bar, grip bar overhand',
      'Pull chest up to bar, keeping body straight',
      'Lower back down with control'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  pistolSquat: {
    id: 'pistolSquat',
    name: 'Pistol Squat (Single Leg)',
    nameShort: 'Pistol Squat',
    muscle: 'Quads, Hamstrings, Glutes, Balance',
    category: 'calisthenics',
    muscleGroup: 'legs',
    type: 'compound',
    formTips: [
      'Balance on one leg, extend other leg forward',
      'Squat down on balancing leg until crease is below knee',
      'Drive back up to standing position'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  bulgarianSplitSquat: {
    id: 'bulgarianSplitSquat',
    name: 'Bulgarian Split Squat',
    nameShort: 'Split Squat',
    muscle: 'Quads, Glutes, Hamstrings',
    category: 'calisthenics',
    muscleGroup: 'legs',
    type: 'compound',
    formTips: [
      'Place back foot on bench behind you',
      'Lunge down on front leg until thigh is parallel',
      'Drive up through front heel'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  lunges: {
    id: 'lunges',
    name: 'Walking Lunges',
    nameShort: 'Lunges',
    muscle: 'Quads, Glutes, Hamstrings',
    category: 'calisthenics',
    muscleGroup: 'legs',
    type: 'accessory',
    formTips: [
      'Step forward, lower hips until back knee is near floor',
      'Keep front knee behind toes',
      'Drive forward into next stride'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  hangingLegRaise: {
    id: 'hangingLegRaise',
    name: 'Hanging Leg Raise',
    nameShort: 'Leg Raise',
    muscle: 'Core, Abs, Hip Flexors',
    category: 'calisthenics',
    muscleGroup: 'core',
    type: 'accessory',
    formTips: [
      'Hang from bar, pull legs up straight to 90 degrees',
      'Lower legs slowly with control — no swinging',
      'Engage abs throughout'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  abWheelRollout: {
    id: 'abWheelRollout',
    name: 'Ab Wheel Rollout',
    nameShort: 'Ab Rollout',
    muscle: 'Core, Abs, Upper Back',
    category: 'calisthenics',
    muscleGroup: 'core',
    type: 'accessory',
    formTips: [
      'Kneel on floor, roll wheel forward with straight arms',
      'Keep core braced, do not let lower back sag',
      'Pull wheel back to starting position using abs'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  plank: {
    id: 'plank',
    name: 'Forearm Plank Hold',
    nameShort: 'Plank',
    muscle: 'Core, Abs, Shoulders',
    category: 'calisthenics',
    muscleGroup: 'core',
    type: 'accessory',
    formTips: [
      'Support weight on forearms and toes',
      'Keep body straight, squeeze glutes and abs',
      'Hold position without letting hips sag'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  russianTwist: {
    id: 'russianTwist',
    name: 'Russian Twist',
    nameShort: 'Russian Twist',
    muscle: 'Core, Obliques',
    category: 'calisthenics',
    muscleGroup: 'core',
    type: 'accessory',
    formTips: [
      'Sit on floor, lean back slightly, float feet',
      'Rotate torso side-to-side, touch hands to floor',
      'Keep movement controlled'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  jumpRope: {
    id: 'jumpRope',
    name: 'Jump Rope',
    nameShort: 'Jump Rope',
    muscle: 'Cardio, Calves, Coordination',
    category: 'cardio',
    muscleGroup: 'cardio',
    type: 'conditioning',
    formTips: [
      'Jump on balls of feet, keep knees soft',
      'Rotate rope using wrists, not arms',
      'Keep jumps low and consistent'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  running: {
    id: 'running',
    name: 'Outdoor / Treadmill Run',
    nameShort: 'Running',
    muscle: 'Cardio, Legs',
    category: 'cardio',
    muscleGroup: 'cardio',
    type: 'conditioning',
    formTips: [
      'Maintain upright posture, gaze forward',
      'Land on midfoot, push off toes',
      'Breathe rhythmically'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  rowingMachine: {
    id: 'rowingMachine',
    name: 'Rowing Machine',
    nameShort: 'Rowing',
    muscle: 'Cardio, Back, Legs, Full Body',
    category: 'cardio',
    muscleGroup: 'cardio',
    type: 'conditioning',
    formTips: [
      'Drive with legs first, then lean back, then pull handle',
      'Return arms, hinge hips forward, bend legs',
      'Keep back flat throughout the stroke'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  burpees: {
    id: 'burpees',
    name: 'Burpees',
    nameShort: 'Burpee',
    muscle: 'Cardio, Legs, Chest, Core',
    category: 'cardio',
    muscleGroup: 'cardio',
    type: 'conditioning',
    formTips: [
      'Drop into squat, jump feet back to pushup plank',
      'Do a pushup, jump feet forward to hands',
      'Explode up into jump, clap overhead'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  legSwings: {
    id: 'legSwings',
    name: 'Leg Swings (Warm-up)',
    nameShort: 'Leg Swings',
    muscle: 'Hip Mobility, Hamstrings',
    category: 'mobility',
    muscleGroup: 'mobility',
    type: 'mobility',
    formTips: [
      'Stand near wall for balance',
      'Swing leg front-to-back with control',
      'Keep torso strict'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  armCircles: {
    id: 'armCircles',
    name: 'Arm Circles (Warm-up)',
    nameShort: 'Arm Circles',
    muscle: 'Shoulder Mobility',
    category: 'mobility',
    muscleGroup: 'mobility',
    type: 'mobility',
    formTips: [
      'Extend arms straight out to sides',
      'Make circular motions forward and backward',
      'Start small, increase diameter'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  catCow: {
    id: 'catCow',
    name: 'Cat-Cow Stretch',
    nameShort: 'Cat-Cow',
    muscle: 'Spine Mobility',
    category: 'mobility',
    muscleGroup: 'mobility',
    type: 'mobility',
    formTips: [
      'Get on hands and knees',
      'Inhale, arch back, look up (Cow)',
      'Exhale, round spine, tuck chin (Cat)'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  worldsGreatestStretch: {
    id: 'worldsGreatestStretch',
    name: "World's Greatest Stretch",
    nameShort: 'WGS Stretch',
    muscle: 'Hips, Thoracic Spine, Hamstrings',
    category: 'mobility',
    muscleGroup: 'mobility',
    type: 'mobility',
    formTips: [
      'Step forward into deep lunge',
      'Place opposite hand on floor, rotate torso',
      'Reach arm to sky, look up'
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  legExtension: {
    id: 'legExtension',
    name: 'Leg Extension Machine',
    nameShort: 'Leg Extension',
    muscle: 'Quads',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'accessory',
    formTips: [
      'Adjust pad to sit just above ankles',
      'Extend legs fully, squeeze quads at top',
      'Lower slowly with control — don\'t let the weight drop',
    ],
    warnings: [],
    startWeight: 20,
    increment: 5,
  },
  hangingKneeRaise: {
    id: 'hangingKneeRaise',
    name: 'Hanging Knee Raise',
    nameShort: 'Knee Raise',
    muscle: 'Core, Abs, Hip Flexors',
    category: 'calisthenics',
    muscleGroup: 'core',
    type: 'accessory',
    formTips: [
      'Hang from bar with straight arms',
      'Raise knees to chest with control',
      'Lower slowly — no swinging',
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  rearDeltFly: {
    id: 'rearDeltFly',
    name: 'Rear Delt Fly',
    nameShort: 'Rear Delt Fly',
    muscle: 'Rear Delts, Upper Back',
    category: 'strength',
    muscleGroup: 'shoulders',
    type: 'accessory',
    formTips: [
      'Use machine (reverse pec dec) or bent-over with dumbbells',
      'Lead with elbows, squeeze shoulder blades together',
      'Control the weight on the way back',
    ],
    warnings: [],
    startWeight: 5,
    increment: 2.5,
  },
  dbShrugs: {
    id: 'dbShrugs',
    name: 'Dumbbell Shrugs',
    nameShort: 'DB Shrugs',
    muscle: 'Traps, Upper Back',
    category: 'strength',
    muscleGroup: 'back',
    type: 'accessory',
    formTips: [
      'Hold dumbbells at sides, stand tall',
      'Shrug shoulders straight up toward ears',
      'Hold at top for 1 second, lower slowly',
    ],
    warnings: [],
    startWeight: 12,
    increment: 2,
  },
  preacherCurl: {
    id: 'preacherCurl',
    name: 'Preacher Curl',
    nameShort: 'Preacher Curl',
    muscle: 'Biceps',
    category: 'strength',
    muscleGroup: 'biceps',
    type: 'accessory',
    formTips: [
      'Sit at preacher bench, upper arms flat on pad',
      'Curl weight up, squeezing biceps at top',
      'Lower fully to feel the stretch — no cheating',
    ],
    warnings: [],
    startWeight: 10,
    increment: 2.5,
  },
  inclineDbCurl: {
    id: 'inclineDbCurl',
    name: 'Incline Dumbbell Curl',
    nameShort: 'Incline DB Curl',
    muscle: 'Biceps (long head)',
    category: 'strength',
    muscleGroup: 'biceps',
    type: 'accessory',
    formTips: [
      'Set bench to 45° incline, arms hanging straight down',
      'Curl dumbbells up without moving upper arms',
      'Stretch fully at bottom for maximum stimulus',
    ],
    warnings: [],
    startWeight: 6,
    increment: 2,
  },
  reverseWristCurl: {
    id: 'reverseWristCurl',
    name: 'Reverse Wrist Curl',
    nameShort: 'Rev. Wrist Curl',
    muscle: 'Forearm Extensors',
    category: 'strength',
    muscleGroup: 'forearms',
    type: 'accessory',
    formTips: [
      'Rest forearms on bench or knees, palms facing down',
      'Curl wrists upward with light weight',
      'Control the descent slowly',
    ],
    warnings: [],
    startWeight: 5,
    increment: 1,
  },
  cableFly: {
    id: 'cableFly',
    name: 'Cable Fly / Pec Dec',
    nameShort: 'Cable Fly',
    muscle: 'Chest',
    category: 'strength',
    muscleGroup: 'chest',
    type: 'accessory',
    formTips: [
      'Set cables at chest height, slight forward lean',
      'Bring handles together in a hugging motion',
      'Squeeze chest at center, stretch wide on return',
    ],
    warnings: [],
    startWeight: 10,
    increment: 2.5,
  },
  hipThrust: {
    id: 'hipThrust',
    name: 'Barbell Hip Thrust',
    nameShort: 'Hip Thrust',
    muscle: 'Glutes, Hamstrings',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'compound',
    formTips: [
      'Upper back on bench, barbell across hips',
      'Drive hips up until torso is parallel to floor',
      'Squeeze glutes hard at top for 2 seconds',
    ],
    warnings: [
      'Use a pad on the barbell for comfort',
    ],
    startWeight: 20,
    increment: 5,
  },
  seatedLegCurl: {
    id: 'seatedLegCurl',
    name: 'Seated Leg Curl',
    nameShort: 'Seated Leg Curl',
    muscle: 'Hamstrings',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'accessory',
    formTips: [
      'Adjust pad to sit above ankles',
      'Curl legs down and back slowly',
      'Squeeze hamstrings at bottom, control the return',
    ],
    warnings: [],
    startWeight: 20,
    increment: 5,
  },
  hipAbduction: {
    id: 'hipAbduction',
    name: 'Hip Abduction Machine',
    nameShort: 'Hip Abduction',
    muscle: 'Glute Medius, Outer Glutes',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'accessory',
    formTips: [
      'Sit upright, pads against outer thighs',
      'Push legs apart against resistance',
      'Squeeze at full extension, return slowly',
    ],
    warnings: [],
    startWeight: 30,
    increment: 5,
  },
  seatedCalfRaise: {
    id: 'seatedCalfRaise',
    name: 'Seated Calf Raise',
    nameShort: 'Seated Calf Raise',
    muscle: 'Soleus, Calves',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'accessory',
    formTips: [
      'Sit with pad on knees, balls of feet on platform',
      'Push up onto toes, squeeze at top',
      'Lower heels below platform for full stretch',
    ],
    warnings: [],
    startWeight: 20,
    increment: 5,
  },
  chestSupportedDbRow: {
    id: 'chestSupportedDbRow',
    name: 'Chest-Supported DB Row',
    nameShort: 'Chest-Sup. Row',
    muscle: 'Upper Back, Lats, Rear Delts',
    category: 'strength',
    muscleGroup: 'back',
    type: 'accessory',
    formTips: [
      'Lie face down on incline bench, arms hanging',
      'Row dumbbells up, squeezing shoulder blades',
      'No body english — pure upper back isolation',
    ],
    warnings: [],
    startWeight: 8,
    increment: 2,
  },
  reverseEzCurl: {
    id: 'reverseEzCurl',
    name: 'Reverse EZ-Bar Curl',
    nameShort: 'Rev. EZ Curl',
    muscle: 'Forearm Extensors, Brachioradialis',
    category: 'strength',
    muscleGroup: 'forearms',
    type: 'accessory',
    formTips: [
      'Grip EZ-bar with overhand (pronated) grip',
      'Curl up with control, elbows pinned to sides',
      'Lower slowly — forearms should burn',
    ],
    warnings: [],
    startWeight: 10,
    increment: 2.5,
  },
  wristCurl: {
    id: 'wristCurl',
    name: 'Wrist Curl',
    nameShort: 'Wrist Curl',
    muscle: 'Forearm Flexors',
    category: 'strength',
    muscleGroup: 'forearms',
    type: 'accessory',
    formTips: [
      'Rest forearms on bench or knees, palms facing up',
      'Curl wrists upward, squeezing forearms',
      'Lower slowly with control',
    ],
    warnings: [],
    startWeight: 5,
    increment: 1,
  },
  gobletSquat: {
    id: 'gobletSquat',
    name: 'Goblet Squat',
    nameShort: 'Goblet Squat',
    muscle: 'Quads, Glutes, Core',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'compound',
    formTips: [
      'Hold a dumbbell at chest with both hands',
      'Squat down keeping elbows inside knees',
      'Push through heels, keep chest up throughout',
    ],
    warnings: [],
    startWeight: 8,
    increment: 2,
  },
  dbRomanianDeadlift: {
    id: 'dbRomanianDeadlift',
    name: 'Dumbbell Romanian Deadlift',
    nameShort: 'DB Romanian DL',
    muscle: 'Hamstrings, Glutes, Lower Back',
    category: 'strength',
    muscleGroup: 'legs',
    type: 'compound',
    formTips: [
      'Hold dumbbells in front of thighs',
      'Hinge at hips, sliding dumbbells down legs',
      'Feel the hamstring stretch, squeeze glutes to return',
    ],
    warnings: [
      'Keep back neutral — do not round',
    ],
    startWeight: 8,
    increment: 2,
  },
  dbOverheadPress: {
    id: 'dbOverheadPress',
    name: 'Dumbbell Overhead Press',
    nameShort: 'DB OHP',
    muscle: 'Shoulders, Triceps',
    category: 'strength',
    muscleGroup: 'shoulders',
    type: 'compound',
    formTips: [
      'Seated or standing, dumbbells at shoulder height',
      'Press straight up, lockout overhead',
      'Lower slowly to starting position',
    ],
    warnings: [],
    startWeight: 6,
    increment: 2,
  },
  deadHang: {
    id: 'deadHang',
    name: 'Dead Hang',
    nameShort: 'Dead Hang',
    muscle: 'Grip, Shoulders, Lats',
    category: 'calisthenics',
    muscleGroup: 'back',
    type: 'accessory',
    formTips: [
      'Grip bar overhand, shoulder-width apart',
      'Hang with arms fully extended, shoulders relaxed',
      'Hold as long as possible with steady breathing',
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  dbRow: {
    id: 'dbRow',
    name: 'Dumbbell Row',
    nameShort: 'DB Row',
    muscle: 'Lats, Upper Back, Biceps',
    category: 'strength',
    muscleGroup: 'back',
    type: 'compound',
    formTips: [
      'One knee and hand on bench, other foot on floor',
      'Row dumbbell to hip, squeezing shoulder blade',
      'Lower with control — full stretch at bottom',
    ],
    warnings: [],
    startWeight: 8,
    increment: 2,
  },
  dbFloorPress: {
    id: 'dbFloorPress',
    name: 'Dumbbell Floor Press',
    nameShort: 'DB Floor Press',
    muscle: 'Chest, Triceps, Shoulders',
    category: 'strength',
    muscleGroup: 'chest',
    type: 'compound',
    formTips: [
      'Lie on floor with knees bent, dumbbells at chest',
      'Press dumbbells up until arms are straight',
      'Lower until triceps touch the floor, pause, press again',
    ],
    warnings: [],
    startWeight: 8,
    increment: 2,
  },
  proneYRaise: {
    id: 'proneYRaise',
    name: 'Prone Y-Raise',
    nameShort: 'Prone Y-Raise',
    muscle: 'Lower Traps, Rear Delts, Rotator Cuff',
    category: 'strength',
    muscleGroup: 'back',
    type: 'accessory',
    formTips: [
      'Lie face down on incline bench',
      'Raise arms in a Y shape overhead with thumbs up',
      'Squeeze lower traps at top, lower slowly',
    ],
    warnings: [],
    startWeight: 2,
    increment: 1,
  },
  scapularPullup: {
    id: 'scapularPullup',
    name: 'Scapular Pull-up',
    nameShort: 'Scapular Pull-up',
    muscle: 'Lower Traps, Lats, Scapular Stabilizers',
    category: 'calisthenics',
    muscleGroup: 'back',
    type: 'accessory',
    formTips: [
      'Hang from bar with straight arms',
      'Without bending elbows, squeeze shoulder blades down and together',
      'Hold the retracted position for 1 second, then release',
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
  bandAssistedPullup: {
    id: 'bandAssistedPullup',
    name: 'Band-Assisted Pull-up',
    nameShort: 'Band Pull-up',
    muscle: 'Lats, Biceps, Core, Grip',
    category: 'calisthenics',
    muscleGroup: 'back',
    type: 'compound',
    formTips: [
      'Loop resistance band over bar and place knee or foot in loop',
      'Pull chin over bar with full range of motion',
      'Progress from thick band to thin band as you get stronger',
    ],
    warnings: [],
    startWeight: 0,
    increment: 0,
  },
};

// Personalized CNS Strength Blueprint template
export const workoutTemplates = {
  custom: {
    name: "CNS Strength Blueprint",
    exercises: [
      { exerciseId: 'squat', sets: 3, reps: 5, amrap: false, restMinutes: 4, notes: 'White-knuckle the bar and squeeze glutes. Rest 4m.' },
      { exerciseId: 'bench', sets: 3, reps: 5, amrap: true, restMinutes: 4, notes: 'Crush the bar, brace stomach. Last set AMRAP. Rest 4m.' },
      { exerciseId: 'row', sets: 3, reps: 5, amrap: false, restMinutes: 3, notes: 'Explode up, 2-sec slow eccentric control. Rest 3m.' },
      { exerciseId: 'ohp', sets: 3, reps: 5, amrap: true, restMinutes: 4, notes: 'Stand tall, squeeze glutes. Last set AMRAP. Rest 4m.' },
      { exerciseId: 'deadlift', sets: 1, reps: 5, amrap: true, restMinutes: 5, notes: 'Single high-effort work set. Pull with perfect form. Rest 5m.' },
    ],
    conditioning: ['farmerWalk', 'kbSwing'],
  }
};

// 3-day weekly schedule: Mon=A, Wed=B, Fri=A, then next week B/A/B
export const weeklySchedule = [
  { day: 0, name: 'Sunday', workout: null, type: 'rest' },
  { day: 1, name: 'Monday', workout: 'A', type: 'training' },
  { day: 2, name: 'Tuesday', workout: null, type: 'rest' },
  { day: 3, name: 'Wednesday', workout: 'B', type: 'training' },
  { day: 4, name: 'Thursday', workout: null, type: 'rest' },
  { day: 5, name: 'Friday', workout: 'A', type: 'training' },
  { day: 6, name: 'Saturday', workout: null, type: 'rest' },
];

export const warmupRoutine = [
  { 
    name: 'Light Cardio (Jump Rope / Brisk Walk)', 
    duration: '3-5 min', 
    instructions: 'Perform 3-5 minutes of light jumping rope, jumping jacks, or brisk walking to raise core body temperature and heart rate. You should build a light sweat.'
  },
  { 
    name: 'Leg Swings (front/back + side/side)', 
    duration: '10 each leg', 
    instructions: 'Stand near a wall for balance. Swing one leg front-to-back 10 times keeping your torso upright, then swing side-to-side 10 times to unlock hip capsule mobility.'
  },
  { 
    name: "World's Greatest Stretch", 
    duration: '5 each side', 
    instructions: 'Step into a deep lunge with your left foot forward. Place right hand flat on floor. Rotate your torso left and reach left hand to the sky. Hold 2 seconds, then swap sides.'
  },
  { 
    name: 'Wall Ankle Mobilizations', 
    duration: '10 each side', 
    instructions: 'Stand facing a wall, toes a few inches away. Keep your heel flat on the floor and drive your knee forward to tap the wall. Repeat 10 times to enhance dorsiflexion.'
  },
  { 
    name: 'Cat-Cow', 
    duration: '10 reps', 
    instructions: 'Get on hands and knees. Inhale, arch your back, look up (Cow). Exhale, round your spine toward the ceiling, tucking your chin (Cat). Move smoothly to lubricate the spine.'
  },
  { 
    name: 'Bodyweight Squats (squat prying)', 
    duration: '10 reps', 
    instructions: 'Squat deep with heels flat. Place elbows inside knees, press knees outward, and rock gently side-to-side to pry open the hips. Stand and repeat 10 times.'
  },
  { 
    name: 'Arm Circles + Band Pass-Throughs', 
    duration: '10 reps', 
    instructions: 'Perform 10 large circles forward and backward with both arms. Grip a resistance band or stick wide, pass it slowly over your head to the back and return to open up shoulders.'
  },
];

export const warmupSets = [
  { label: 'Warm-up 1', load: '40%', reps: 10, purpose: 'Movement rehearsal' },
  { label: 'Warm-up 2', load: '60%', reps: 5, purpose: 'Build connection' },
  { label: 'Warm-up 3', load: '75%', reps: 3, purpose: 'Increase intensity' },
  { label: 'Warm-up 4', load: '85%', reps: 2, purpose: 'Prime nervous system' },
];

export const plateauTechniques = [
  {
    name: 'Isometrics (Static Holds)',
    description: 'Hold the bar completely still at your weakest point for 5 seconds.',
    when: 'When you fail at a specific point in the lift',
    how: 'Pause at sticking point, hold for 5-7 seconds under tension',
  },
  {
    name: 'Eccentric Negatives',
    description: 'Lower 30% more weight than you can lift over 5 agonizing seconds.',
    when: 'When you need to train beyond your concentric max',
    how: 'Load 105% 1RM, spotter helps up, 5s controlled lower',
  },
  {
    name: 'Cluster Sets',
    description: 'Break sets into single reps with 20-second micro-rests.',
    when: 'When you can\'t complete target reps at a weight',
    how: '1 rep → rack → 20 sec → 1 rep → rack → 20 sec...',
  },
  {
    name: 'Pause Reps',
    description: 'Kill all momentum with a 2-second dead stop at the bottom.',
    when: 'When momentum is doing the work instead of your muscles',
    how: 'Lower weight, pause 2 full seconds at bottom, explode up',
  },
];

export const irradiationChecklist = [
  { step: 1, text: 'CRUSH the bar — white-knuckle grip' },
  { step: 2, text: 'ROOT your feet — spread the floor' },
  { step: 3, text: 'BIG BELLY BREATH — Valsalva maneuver' },
  { step: 4, text: 'BRACE your core — prepare for a punch' },
  { step: 5, text: 'SQUEEZE your glutes' },
  { step: 6, text: 'PULL lats tight' },
  { step: 7, text: 'LIFT with FULL body tension' },
];

export const deloadConfig = {
  cycleWeeks: 6,
  volumeReduction: 0.5,
  intensityReduction: 0.6,
  tips: [
    'Keep going to the gym — maintain the habit',
    'Focus on mobility, stretching, foam rolling',
    'Prioritize sleep — aim for 9+ hours',
    'Eat well — don\'t cut calories during deload',
    'Do NOT train to failure',
    'Do NOT add new exercises',
    'Do NOT feel guilty — this IS the program',
  ],
};
